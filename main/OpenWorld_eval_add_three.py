...
def insert_each_pattern_v2(result_list,data,label,model,tra_len):
    ...
    for i in range (monitored_class_num):
        ...
        if min_tpr >= 0.1 :
            ...
            index = heapq.nsmallest(3, range(len(tpr_pattern_list)), tpr_pattern_list.__getitem__)
            ...
            for k in range(temp_data.shape[0]):
                # 获取第一次次插入的index
                insert_index0 = eval(result_list['insert_indexes'][min_tpr_row])
...
def run():
    ...
    print('r-precision:','train_dataset(95*800)_add_three:')
